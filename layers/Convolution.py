...
class Conv2D(Layer):
    def __init__(self, filter_nums: int, filter_size: Tuple, input_shape: Tuple = None, stride: int = 1, padding: str = 'VALID', activation = 'linear',initializer = 'Normal', **kwargs):
    ...
    def _initial_params(self, *args):
        ...
        W = Variable(self.initializer((self.filter_nums, n_C_prev, filter_h, filter_w)))
        b = Variable(cp.random.randn(self.filter_nums, ))
    ...
    #     # pad
    #     X_padded = cp.pad(X, ((0, 0), (0, 0), self.pad_size, self.pad_size), 'constant')
    #
    ...
    #     print(X.itemsize)
    #     X_stride = cp.lib.stride_tricks.as_strided(X_padded, shape=shape, strides=strides)
    ...
    #     output = cp.ascontiguousarray(out)
    #
    #     self.output_tensor = self.activator._forward(output)
    #
    #     # store it for bp
    ...
    def backward(self):
        ...
        if b.require_grads:
            b.grads += cp.sum(grads, axis=(0, 2, 3))
...
class ZeroPadding2D(Layer):
    def __init__(self, pad_size: Tuple, **kwargs):
...
class MaxPooling2D(Layer):
    def __init__(self, pool_size: Tuple, stride: int = None, **kwargs):
    ...
    def connect(self, prev_layer: Layer):
        ...
        if pool_h == pool_w == self.stride and n_H_prev % pool_h == 0:
            self.mode = 'reshape'
        else:
            self.mode = 'im2col'
...
class MeanPooling2D(Layer):
    ...
    def __call__(self, prev_layer: Layer):
